...
| **Issue #6 resolution**                | User can author a single plain file instead of a folder                                | `/review` from `.pi/prompt-composer/prompts/review.md` works when file opts into composer handling |
...
**Preconditions:** The author has a composer prompt root such as `~/.pi/agent/prompt-composer/prompts/` or `.pi/prompt-composer/prompts/`. Root-level native Pi prompt files such as `.pi/prompts/review.md` stay native Pi resources and are not valid composer-owned flat prompt locations.
...
2. **Prompt kind detection** — distinguish native Pi prompts, composer flat prompts, and composer grouped prompts through explicit frontmatter and native-hidden layout.
...
Composer must discover eligible flat `.md` files directly under composer-owned prompt roots and register each as a slash command. Valid roots are native-hidden paths that Pi does not load as native prompt templates:

* project: `.pi/prompt-composer/prompts/*.md`
* user: `~/.pi/agent/prompt-composer/prompts/*.md`
* bundled/package-owned: package files scanned by the extension itself, not exposed through `pi.prompts`

Composer must not use root-level native Pi prompt paths such as `.pi/prompts/*.md` or `~/.pi/agent/prompts/*.md` for flat composer prompt files. Pi auto-discovers those files as native prompt templates, which would create duplicate command visibility and a native execution path.
...
```gherkin
Given .pi/prompt-composer/prompts/review.md contains frontmatter type: prompt and engine: liquid
...
And the docs tell the author to move it to .pi/prompt-composer/prompts/review.md
```
...
```gherkin
Given .pi/prompt-composer/prompts/review.md declares engine: liquid
...
And composer has .pi/prompt-composer/prompts/review.md with type: prompt
When Pi reloads
...
| Composer flat prompts shadow native Pi prompts unexpectedly           | High     | Medium     | Keep flat composer prompt files in native-hidden `.pi/prompt-composer/prompts/` roots; require `type: prompt`; warn on duplicate composer commands.                        |
| Disabled prompts reappear from lower-precedence or native resources   | High     | Medium     | Treat higher-precedence disabled composer resources as tombstones; keep valid flat composer files out of native prompt roots; block only misplaced/legacy paths if needed. |
...
* Valid flat composer prompt files live in native-hidden composer roots (`.pi/prompt-composer/prompts/` and `~/.pi/agent/prompt-composer/prompts/`) so the same file cannot also appear or execute as a native Pi prompt.
...
**Decision:** Keep author-facing `type` markers for composer-owned resources. A grouped folder is composer-owned only when `_index.md` declares `type: group`. A flat file in a native-hidden composer prompt root is composer-owned only when it declares `type: prompt`. `engine` remains a renderer selector, not an ownership marker.

**Rationale:** The hard gate is valuable. It prevents accidental command registration from documentation-like folders, separates “composer owns this resource” from “render this with Liquid,” and gives enable/disable semantics a stable target. This also leaves room for composer-owned `engine: pi` prompts that need typed args, dispatch modes, shell preprocessing, or disabled-path behavior without pretending they are native Pi prompts. Native-hidden roots are required because Pi's native prompt inventory has no extension-level filter for removing already discovered `.pi/prompts/*.md` files.
...
| `src/discovery.ts`                           | New          | FR-1, FR-2, FR-6, FR-8       | Discover native-hidden flat composer prompts and grouped prompts; handle precedence and warnings. |
...
| `examples/prompt-composer/prompts/review.md` | New          | FR-1, FR-3, FR-4, FR-9       | Flat enhanced prompt example in native-hidden composer layout.                                    |
...
* Composer-owned flat prompt files must therefore live outside native prompt roots, under `.pi/prompt-composer/prompts/` or `~/.pi/agent/prompt-composer/prompts/`.
...
4. **Liquid rendering and flat discovery** — add `engine: liquid`, named render context, safe filter allowlist, snapshots, and flat native-hidden `type: prompt` registration.
...
| Q8 | Where should composer-owned flat prompt files live so Pi does not also load them as native prompts?                     | Victor Software House | 2026-05-08 | Use native-hidden composer roots: `.pi/prompt-composer/prompts/` and `~/.pi/agent/prompt-composer/prompts/`. | Resolved |
...
1. Create `.pi/prompt-composer/prompts/review.md` with `type: prompt`, `engine: liquid`, typed args, an `if` block, and a `for` loop.
