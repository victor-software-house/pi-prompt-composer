...
- Keep `type: group` out of new generated `_index.md` examples unless documenting legacy compatibility.
...
- Add validation decision ladder:
  ...
  3. `values`
  4. `default`
  5. declarative `validate.*` once implemented
  6. prompt-body verification for semantic checks
