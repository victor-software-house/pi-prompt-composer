...
Make `pi-prompt-composer` a real publishable Pi package without changing its grouped-routing runtime. The implementation focuses on package polish and distribution readiness: rewrite `README.md` for fast user onboarding, ship copyable example prompts, add a reproducible preview-image generator and committed preview asset, complete npm/GitHub package metadata (including gallery image support), strengthen the publish workflow so tests gate releases, and document both manual end-to-end Pi validation and the one-time npm trusted-publishing bootstrap needed before fully automated releases.
...
**Constraints**: Preserve current runtime behavior in `extensions/index.ts`; keep flat Pi prompt templates unaffected; examples must reflect current limitations honestly (no guided argument collection, no shell substitution); first npm publish must be bootstrapped manually before trusted publishing can work; steady-state publish workflow should be tokenless (OIDC + `GITHUB_TOKEN` only); committed preview asset must be stable enough for both README embedding and Pi package gallery metadata  
**Scale/Scope**: Add package-facing assets and docs (`examples/`, `assets/`, `scripts/`, maintainer docs), update `package.json` and `.github/workflows/publish.yml`, and keep the runtime code path unchanged
...
- `extensions/index.ts` remains the only runtime entrypoint and is intentionally unchanged by this feature.
...
```text
extensions/
└── index.ts                     # Unchanged runtime entrypoint
...
| File | Change | Reason |
|------|--------|--------|
| `README.md` | Rewrite for user-friendly onboarding; add hero description, preview image, quick-start, example copy instructions, honest current-limitations notes | FR-001 to FR-005 |
...
1. **Package metadata and workflow slice**
...
2. **Examples and onboarding docs slice**
...
4. **Maintainer workflow docs slice**
