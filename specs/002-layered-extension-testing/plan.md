...
**Testing**: vitest (`pnpm run test` = `vitest --run`, `pnpm run test:watch` = `vitest`)  
**Target Platform**: Developer machine running pnpm and Pi  
...
**Constraints**: Must preserve current runtime behavior in `extensions/index.ts`; no new runtime source directories; all refactoring limited to adding named exports; `pnpm run test` must be enforced in lefthook pre-push but not pre-commit  
...
- Documentation and workflow updates required: `README.md` (verification guidance), `AGENTS.md` (verification commands and removal of the legacy "no test suite yet" statement), `lefthook.yml` (add `pnpm run test` to pre-push), and `package.json` (test scripts and dev deps).
- The verification workflow becomes: `pnpm install`, `pnpm run fix`, `pnpm run typecheck`, `pnpm run lint`, `pnpm run test`, with `pnpm run test` also enforced by lefthook pre-push.
...
lefthook.yml             # Add pnpm run test to pre-push hook
```
...
| `README.md` | Add `pnpm run test` to verification section | CC-003, FR-007 |
| `AGENTS.md` | Add `pnpm run test` to verification workflow and remove the legacy "There is no test suite yet" sentence | CC-003, FR-007 |
| `lefthook.yml` | Add `pnpm run test` to the pre-push hook without slowing pre-commit | CC-003, FR-007 |
...
7. **Validation slice**: Run full verification workflow: `pnpm install`, `pnpm run fix`, `pnpm run typecheck`, `pnpm run lint`, `pnpm run test`. Timing goals remain advisory and are not enforced as pass/fail gates.

## Validation Strategy

- **Static validation**: `pnpm install`, `pnpm run fix`, `pnpm run typecheck`, `pnpm run lint`
- **Automated test validation**: `pnpm run test` (all three layers)
...
- **Coexistence check**: FR-008 — verify `pnpm run typecheck` and `pnpm run lint` still pass after adding test infrastructure
- **Hook enforcement**: confirm `lefthook.yml` runs `pnpm run test` on pre-push while keeping pre-commit unchanged
